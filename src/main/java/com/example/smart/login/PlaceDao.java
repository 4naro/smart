...
//		WHERE EMAIL = ? AND PASSWORD = ?;
		...
		public Place get(String email, String password) { // get per ritornare uno User
			...
			try (PreparedStatement stmt = connection.prepareStatement(GET_BY_USERNAME_AND_PASSWORD)) { //
				...
				try (ResultSet rs = stmt.executeQuery()) {
					if (rs.next()) {// se c'è una riga 'email' + password allora avrò la riga corrispondente
						...
						return new Place(rs.getInt(1), rs.getString(2), rs.getString(3)); // creami al volo uno User e
																							// ricordalo; qui
						// abbiamo
